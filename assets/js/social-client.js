...
(function(){
  ...
  function toast(msg){
    ...
    let el = document.getElementById("v369InnToast");
    if(!el){
  ...
  const avatarText = name => esc((String(name || "夢").trim()).slice(0,1) || "夢");
  function timeText(v){
    if(!v) return "";
    const d = new Date(v);
  ...
  const isInnPage = () => location.hash === "#inn" || !!document.querySelector("#page-inn.active");

  function renderComments(post){
    const comments = Array.isArray(post.comments) ? post.comments : [];
    if(!comments.length) return "";
    return `<div class="inn-comments">${comments.map(c=>`<div class="inn-comment"><b>${esc(c.author_name || "會員")}</b><span>${esc(c.comment_text || c.content || "")}</span></div>`).join("")}</div>`;
  }
