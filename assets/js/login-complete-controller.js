...
(function(){
  if(window.__dreamLoginCompleteControllerV384)return;window.__dreamLoginCompleteControllerV384=true;console.info("[DreamLogin v392] controller loaded");
  ...
  async function login(btn,ev){if(ev)cancelEvent(ev,true);if(running)return false;btn=btn||$("#front_login_btn")||$("#v44_login_btn")||document.querySelector(".login-main-btn-v116");const data=inputs();const r=role();console.info("[DreamLogin v392] login triggered",{role:r,hasUsername:!!data.username,hasPassword:!!data.password,button:btn&&(btn.id||btn.className||btn.textContent)});if(!data.username||!data.password){toast("請輸入帳號與密碼");return false;}running=true;setLoading(btn,true);toast("正在登入中，請稍候...",true);const payload={username:data.username,password:data.password,account:data.username,email:data.username};const action=r==="companion"?"companion_login":"login";let res=await post(action,payload);if(r==="member"&&(!res||!res.ok)&&(res.status===400||res.status===404))res=await post("member_login",payload);window.DreamLoginDebug={version:"v392-login-fast-public-api",role:r,action,payload:{username:data.username,account:data.username,email:data.username},response:res};if(!res||!res.ok){running=false;setLoading(btn,false);toast((res&&(res.message||res.error))||"登入失敗");console.warn("[DreamLogin v392] login failed",res);return false;}const user=res.user||res.companion||res.data||{username:data.username,display_name:data.username};persist(r,user);toast(r==="companion"?"陪玩登入成功，正在前往陪玩中心...":"登入成功，正在前往會員中心...",true);console.info("[DreamLogin v392] login success",{role:r,user});setTimeout(()=>{running=false;setLoading(btn,false);go(r==="companion"?"companion-home":"member");setTimeout(hideLoginToastV385,650);},260);return false;}
