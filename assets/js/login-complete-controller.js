...
(function(){
  ...
  function toast(msg,sticky){let el=$("#dreamLoginToast");if(!el){el=document.createElement("div");el.id="dreamLoginToast";el.style.cssText="position:fixed;left:50%;bottom:112px;transform:translateX(-50%);width:min(88%,390px);padding:12px 15px;border-radius:16px;border:1px solid rgba(255,221,190,.42);background:rgba(72,28,50,.98);color:#fff;text-align:center;font-size:14px;font-weight:900;z-index:2147483647;box-shadow:0 14px 28px rgba(0,0,0,.45);opacity:1;transition:.2s";document.body.appendChild(el);}el.textContent=String(msg||"");el.style.opacity="1";clearTimeout(el._t);if(!sticky)el._t=setTimeout(()=>{el.style.opacity="0";},2400);}
  ...
  async function login(btn,ev){if(ev)cancelEvent(ev,true);if(running)return false;btn=btn||$("#front_login_btn")||$("#v44_login_btn")||document.querySelector(".login-main-btn-v116");const data=inputs();const r=role();if(window.DREAM_API_DEBUG) console.info("[DreamLogin v404] login triggered",{role:r,hasUsername:!!data.username,hasPassword:!!data.password,button:btn&&(btn.id||btn.className||btn.textContent)});if(!data.username||!data.password){toast("請輸入帳號與密碼");return false;}running=true;setLoading(btn,true);toast("正在登入中，請稍候...",true);const payload={username:data.username,password:data.password,account:data.username,email:data.username};const action=r==="companion"?"companion_login":"login";let res=await post(action,payload);if(r==="member"&&(!res||!res.ok)&&(res.status===400||res.status===404))res=await post("member_login",payload);window.DreamLoginDebug={version:"v404-formal-release",role:r,action,payload:{username:data.username,account:data.username,email:data.username},response:res};if(!res||!res.ok){running=false;setLoading(btn,false);toast((res&&(res.message||res.error))||"登入失敗");if(window.DREAM_API_DEBUG) console.warn("[DreamLogin v404] login failed",res);return false;}const user=res.user||res.companion||res.data||{username:data.username,display_name:data.username};persist(r,user);toast(r==="companion"?"陪玩登入成功，正在前往陪玩中心...":"登入成功，正在前往會員中心...",true);if(window.DREAM_API_DEBUG) console.info("[DreamLogin v404] login success",{role:r,user});setTimeout(()=>{running=false;setLoading(btn,false);go(r==="companion"?"companion-home":"member");setTimeout(hideLoginToast,650);},260);return false;}
